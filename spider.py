                ...
                for k in range(len(board)):
                    if (m < (len(board)-k)):
                        if board[n,(len(board)-k-1)] == True:
                            boardx = np.ones((len(board), len(board)), dtype=np.bool_)
                    ...
                    if (n-k >= 0) & (m < (len(board)-k)) :
                            if board[n-k,m+k] == True:
                                    ...
                                    for x in range(k+1):
                                            boardx[n-x,m+x] = False
                                    listofallmoves.append(boardx)
    listofallmoves = np.bool_(listofallmoves)
    listofallmoves = listofallmoves.reshape(listofallmoves.shape[0], listofallmoves.shape[1]*listofallmoves.shape[2])
